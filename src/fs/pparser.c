...
static int pathparser_path_valid_fromat(const char* filename) {

    int len = strnlen(filename, OMENOS_MAX_PATH);
    return (len >= 3 && isdigit(filename[0]) && memcmp((void*) &filename[1], ":/",  2) == 0);
}

static int pathparser_get_drive_by_path(const char** path) {

    if (!pathparser_path_valid_fromat(*path)) {
...
static const char* pathparser_get_path_part(const char** path) {
    ...
    char* result_path_part = kzalloc(OMENOS_MAX_PATH);
    int i = 0;
...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path) {
    ...
    if (strlen(path) > OMENOS_MAX_PATH) {
        goto out;
    }
